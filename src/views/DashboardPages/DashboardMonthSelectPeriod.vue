<template>
  <ion-page>
    ...
    <ion-content class="ion-padding" ref="content">
      <div v-for="(month, index) in months" :key="month.name" :id="'month-' + index" class="month">
...
export default {
  ...
  setup() {
    ...
    const receivedToDate = new Date(route.params.todate);
...
initNetworkListener();
  ...
  created() {
    ...
    // Automatically set receivedFromDate and receivedToDate as selected
    if (this.receivedFromDate) {
      this.fromDate = this.receivedFromDate;
    }
    if (this.receivedToDate) {
      this.toDate = this.receivedToDate;
    }
  },
...
if (!this.receivedFromDate) {
  console.error('receivedFromDate is not set');
  return;
}

const selectedMonthIndex = this.receivedFromDate.getMonth();
